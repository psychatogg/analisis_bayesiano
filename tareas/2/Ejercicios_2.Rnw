...
\\
La distribución $f(\theta \mid y)$, con media aproximada de 0,3. \\
\\
Con un $95 \%$ de confianza, el verdadero valor de $\theta$ se encuentra entre -1,06 y 1,71.\\
\\
La autocorrelación disminuye al auumentar la distancia entre valores, es decir, las observaciones de la serie están correlacionadas positivamente con las observaciones anteriores. \\
\\
3. \\
\end{document}