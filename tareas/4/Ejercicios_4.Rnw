...
\begin{document}
<<include=FALSE,echo=FALSE,warnings=FALSE,tidy=TRUE>>=
options(width=60)
library(formatR)
@
Resolución \\
...
@
El modelo 2 presenta peor ajuste y es más sencillo. Por otra parte, su IC posterior para mu no incluye el 0 por lo que con un 95\% de confianza no podemos asumir que la diferencia entre ambos grupos sea nula. Elijo por lo tanto el modelo 1. \\
...
2.5. \\
El modelo 2 es más sencillo que el 1, y presenta mayor ajuste. Elijo por lo tanto el modelo 2.
\end{document}